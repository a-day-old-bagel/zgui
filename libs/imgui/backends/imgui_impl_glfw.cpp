...
{
    for (ImGui_ImplGlfw_WindowToContext& entry : g_ContextMap) if (entry.Window == window) { g_ContextMap.erase_unsorted(&entry); break; }
    // zig-gamedev change: Explicit destructor call once empty to prevent warnings about static memory usage on exit
    // This will cause problems if any use case exists where g_ContextMap is completely emptied and then used again.
    if (g_ContextMap.empty()) g_ContextMap.~ImVector<ImGui_ImplGlfw_WindowToContext>();
}
